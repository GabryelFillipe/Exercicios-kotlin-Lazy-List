package com.example.listas.repository
...
fun getGamesByStudio(studio: String): List<Game>{
  return  getAllGames().filter { game ->  game.studio.startsWith(studio, ignoreCase = true) }
}
fun getGamesByTitle(title: String): List<Game>{
    return  getAllGames().filter { game ->  game.title.startsWith(title, ignoreCase = true) }
}